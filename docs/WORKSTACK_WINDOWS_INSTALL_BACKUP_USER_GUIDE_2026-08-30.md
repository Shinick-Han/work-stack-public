...
1. Download `WorkStack-Setup-1.0.0.ps1` and its adjacent
   `WorkStack-Setup-1.0.0.ps1.sha256` file.
2. Open PowerShell in the download folder.
3. Verify that the setup bytes match the sidecar before running them:

   ```powershell
   $expected = (Get-Content .\WorkStack-Setup-1.0.0.ps1.sha256 -Raw).Split(' ', 2)[0]
   $actual = (Get-FileHash .\WorkStack-Setup-1.0.0.ps1 -Algorithm SHA256).Hash.ToLowerInvariant()
   ...
   ```powershell
   .\scripts\windows\Test-WorkStackSetup.ps1 -SetupPath .\WorkStack-Setup-1.0.0.ps1
   ```

4. Run:

   ```powershell
   powershell -ExecutionPolicy Bypass -File .\WorkStack-Setup-1.0.0.ps1
   ```
