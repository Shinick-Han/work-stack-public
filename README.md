...
Run the generated `.artifacts\WorkStack-Setup-1.0.0.ps1`. The target machine needs neither
...
The builder also writes `WorkStack-Setup-1.0.0.ps1.sha256`. Keep it beside the setup artifact
and verify both the filename and digest before execution:

```powershell
.\scripts\windows\Test-WorkStackSetup.ps1 `
  -SetupPath .\.artifacts\WorkStack-Setup-1.0.0.ps1
```
